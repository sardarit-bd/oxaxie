...
//           Card Information
//         </label>
//         <div className="px-4 py-3 border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-[#F59F0A] focus-within:border-transparent">
...
function PaymentForm({ plan, onSuccess, onCancel }) {
  ...
  // ❌ REMOVED useEffect - no longer initializing on modal open
  // Payment will ONLY initialize when Pay button is clicked
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // ✅ Step 1: Initialize payment HERE (when Pay button clicked)
