...
export async function POST(request) {
  try {
    ...
    if (token) {
      ...
      try {
        const saveUserMsg = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/chat-messages`, {
          method: 'POST',
          headers: {
...
Always conclude responses by asking if they have any questions or if there's anything specific they'd like to explore further.`;
    ...
    if (token) {
      ...
      try {
        const saveAiMsg = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/chat-messages`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
